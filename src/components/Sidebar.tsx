...
import { Home, FolderOpen, User, Mail, FileText, Moon, Sun, Github, Linkedin, Users, Award } from 'lucide-react';
...
export function Sidebar() {
    ...
    return (
        <aside className="flex w-[250px] shrink-0 flex-col border-r border-border-color bg-sidebar transition-all duration-300 h-screen overflow-hidden">
            <div className="flex flex-col h-full p-6">
                ...
                <nav className="flex flex-col gap-1">
                    <NavLink to="/" className={({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? 'bg-primary/10 text-primary shadow-sm shadow-primary/5' : 'text-text-muted hover:bg-slate-800/50 hover:text-text-main'}`}>
                        <Home size={20} /> Home
                    </NavLink>
                    <NavLink to="/about" className={({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? 'bg-primary/10 text-primary shadow-sm shadow-primary/5' : 'text-text-muted hover:bg-slate-800/50 hover:text-text-main'}`}>
                        <User size={20} /> About
                    </NavLink>
                    <NavLink to="/projects" className={({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? 'bg-primary/10 text-primary shadow-sm shadow-primary/5' : 'text-text-muted hover:bg-slate-800/50 hover:text-text-main'}`}>
                        <FolderOpen size={20} /> Projects
                    </NavLink>
                    <NavLink to="/achievements" className={({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? 'bg-primary/10 text-primary shadow-sm shadow-primary/5' : 'text-text-muted hover:bg-slate-800/50 hover:text-text-main'}`}>
                        <Award size={20} /> Achievements
                    </NavLink>
                    <NavLink to="/contact" className={({ isActive }) => `flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? 'bg-primary/10 text-primary shadow-sm shadow-primary/5' : 'text-text-muted hover:bg-slate-800/50 hover:text-text-main'}`}>
                ...
                <div className="flex flex-col gap-4 mt-6">
                    <a href="/resume.pdf" target="_blank" rel="noopener noreferrer" className="flex h-11 w-full items-center justify-center gap-2 rounded-lg bg-primary px-4 text-sm font-bold text-white shadow-lg shadow-primary/20 transition-transform hover:scale-[1.02] active:scale-95">
