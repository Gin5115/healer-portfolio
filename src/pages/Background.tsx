...
export function Background() {
    ...
    if (loading) {
        return <div className="text-text-main text-center py-12">Loading background details...</div>;
    }
        ...
        <div className="max-w-4xl mx-auto w-full flex flex-col gap-12 pb-12">
            ...
            <div className="flex flex-col gap-4 border-b border-border-color pb-8">
                <div className="flex items-center gap-2 text-primary mb-2">
                    <Briefcase size={20} />
                    <span className="font-mono text-sm font-bold tracking-wider">// BACKGROUND</span>
                </div>
                <h2 className="text-3xl font-black text-text-main sm:text-4xl">Education & Experience</h2>
                <p className="text-lg leading-relaxed text-text-muted">
                    My academic journey and professional career path.
                </p>
            </div>
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    {education.map((edu) => (
                        <div
                            ...
                            className="group flex flex-col gap-3 rounded-xl border border-border-color bg-[var(--bg-project-card)] backdrop-blur-md p-6 shadow-sm transition-all hover:border-primary/50 cursor-pointer hover:shadow-lg hover:translate-y-[-2px]"
                        >
                ...
                <div className="relative border-l border-border-color ml-3 space-y-8 pl-8 py-2">
                    {experience.map((exp) => (
                        <div
                            ...
                            className="group relative flex flex-col gap-2 p-4 -ml-4 rounded-lg hover:bg-card-hover transition-colors cursor-pointer border border-transparent hover:border-border-color/50"
                        >
                            <span className="absolute -left-[23px] top-6 flex h-5 w-5 items-center justify-center rounded-full bg-card ring-4 ring-background border border-primary group-hover:bg-primary transition-colors"></span>
                            ...
                            <div className="mt-1 text-sm font-bold text-primary hover:text-primary/80 transition-colors inline-flex items-center gap-1">
                                View Details <ArrowRight size={14} />
                            </div>
                        </div>
                    ))}
